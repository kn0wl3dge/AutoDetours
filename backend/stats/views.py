...
def getTimeLineChart(model, field):
    ...
    return [{"t": str(d["datetimefield"]), "y": d["count_by_date"]}
            for d in dates]
        ...
        all_data = {
            "count": {
                ...
                "total_malwares_analyzed": Malware.objects.filter(
                    state=MalwareState.ANALYZED).count(),
                "distinct_labels_number": Malware.objects.distinct("label").count(),
                "workers_number": Worker.objects.count()},
            "malwares_repartition": getStateRepartition(
                Malware,
                MalwareState),
            "workers_repartition": getStateRepartition(
                Worker,
                WorkerState),
            "malware_import_timeline": getTimeLineChart(
                Malware,
                "date"),
